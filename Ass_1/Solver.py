...
# Each block contains atmost n numbers
def each_block_atmost_one_no():
    for x in range(2*k) :
        for y in range(k) :
            for c in range(n) :
                    ...
                    for y1 in range(y*k,(y+1)*k) :
                        for i2 in range(x*k,(x+1)*k) :
                            for y2 in range(y*k,(y+1)*k) :
                                if i1==i2 and y1==y2 :
                                    continue 
                                a = create_no1(i1+1,y1+1,c+1)
                                b = create_no1(i2+1,y2+1,c+1)
                                solver.add_clause([-a,-b])
